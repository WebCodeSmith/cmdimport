...
export default function AdminPage() {
  ...
  const abrirModalPrecificacao = (produto: ProdutoComprado) => {
    showToast('O gerenciamento de preços agora é feito na aba Precificação.', 'info')
  }
  ...
  return (
    <>
      {/* Welcome Section */}
      <div className={`mb-6 transition-all duration-300 ${sidebarEdicao || modalDistribuicao ? 'pr-80' : ''}`}>
      ...
      {/* Stats Cards */}
      <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 transition-all duration-300 ${sidebarEdicao || modalDistribuicao ? 'pr-80' : ''}`}>
      ...
      {/* Tab Content */}
      <div className={`bg-white rounded-lg shadow-sm border border-gray-200 p-6 transition-all duration-300 ${sidebarEdicao || modalDistribuicao ? 'mr-80' : ''}`}>
      ...
      {/* Sidebar de Edição */}
      <div className={`fixed top-16 right-0 h-[calc(100vh-4rem)] w-80 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out z-40 border-l border-gray-200 ${sidebarEdicao ? 'translate-x-0' : 'translate-x-full'
        }`}>
      ...
      <div className={`fixed top-16 right-0 h-[calc(100vh-4rem)] w-full sm:w-80 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out z-40 border-l border-gray-200 ${modalDistribuicao ? 'translate-x-0' : 'translate-x-full'
        }`}>
        <div className="h-full flex flex-col">
          {/* Header do Sidebar */}
